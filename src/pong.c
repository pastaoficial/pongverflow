...
#define EXIT_GAME 0x706f6b65
#define MENU_DOWN 0x79747261
#define MENU_UP 0x75636573
#define MENU_ENTER 0x79746972
#define AUTH 0x6b636168
#define PALLETE_UP 0x6e6f7461
#define PALLETE_DOWN 0x69726976
#define GAME_RESTART 0x33313030
#define CHIQUITOLINE 0x6e6f7264
#define ADVISE 0x65706570
...
int ai_names_c = 8;
char* ai_names[8] = {"malware", "virus", "worm", "trojan",\
"ransomware", "exploit", "adware", "spyware"};
    ...
    while ((key = getch()) != 'q'){
			...
			switch (packet->code) {
				...
				case PALLETE_DOWN:
                	...
                	if (packet->joystick == 1) {
                		...
                		if ((paddle1->y + paddle1->vel + paddle1->width)  == game->height){
                    		move_paddle_xy(paddle1, paddle1->x, game->height);
                		} else {
                		...
                		if (((check_pad.y + check_pad.width) - paddle2->vel) == game->height){
                    		move_paddle_xy(paddle2, paddle2->x, game->height);
                		} else {
				...
				case GAME_RESTART:
                	if (game_won){
						...
						game->p1_score = 0;
						game->p1_name = "anonymous";
						game->p1_aictrl = 1;
				
						game->p2_score = 0;
						game->p2_name = "anonymous";
						game->p2_aictrl = 1;
                    	return 0;
                	}
                	break;
				...
				case CHIQUITOLINE:
					if((packet->joystick + 1) % 2 == 1) {
						paddle1->width -= 2;
					} else { paddle2->width -= 2; }

				case ADVISE:
        			chat_win = newwin(3, game->height, game->width - 3, 0);
					
					snprintf(message, 200, packet->advise);
					mvwaddstr(chat_win, 1, 1, message);
        			wrefresh(chat_win);
			}
		}
        ...
        if (game->p1_score == game->max_score){
			...
			if(!game->p1_aictrl) {
				...
				sprintf(query, "SELECT score FROM scoreboard WHERE nick='%s'", game->p1_name);
				...
				int error;
				int score = (game->p1_score - game->p2_score) * game->difficulty;
    			if (rc == SQLITE_ROW) {
        			int points = atoi(sqlite3_column_text(res, 0));

					sprintf(query, "UPDATE scoreboard SET score='%d' WHERE nick='%s'", points+score, game->p1_name);
					error = sqlite3_exec(db, query, 0, 0, 0);
    			} else {
					sprintf(query, "INSERT INTO scoreboard (nick, score) VALUES ('%s', %d)", game->p1_name, score);
					error = sqlite3_exec(db, query, 0, 0, 0);
				}
			...
			if(!game->p2_aictrl) {
				...
				sprintf(query, "SELECT score FROM scoreboard WHERE nick='%s'", game->p2_name);
    			rc = sqlite3_prepare_v2(db, query, -1, &res, 0);    
				...
				int error;
				int score = game->p2_score - game->p1_score;
    			if (rc == SQLITE_ROW) {
        			int points = atoi(sqlite3_column_text(res, 0));

					sprintf(query, "UPDATE scoreboard SET score='%d' WHERE nick='%s'", points+score, game->p2_name);
					error = sqlite3_exec(db, query, 0, 0, 0);
    			} else {
					sprintf(query, "INSERT INTO scoreboard (nick, score) VALUES ('%s', %d)", game->p2_name, score);
					error = sqlite3_exec(db, query, 0, 0, 0);
				}
...
void *receive_client(void *arg)
{
	...
	Packet *packet;
	int joystick = counter;
	
	do {
		read(cl, &code, sizeof(int));
		...
		switch (code) {
			case AUTH:
				...
				if (length < 20) {
					...
					if(i != length) {
						packet->username = (char *)malloc(sizeof(char)*6);
						strcpy(packet->username, "hax0r");
					} else { packet->username = buf; }

					pthread_mutex_lock(&lock);
					g_queue_push_tail(shared_data, (gpointer)packet);
					pthread_mutex_unlock(&lock);
				}
...
int main_menu(WINDOW* screen)
{
	...
	int randnum = randint(ai_names_c);
    char* first_name = ai_names[randnum];
	char* second_name = ai_names[++randnum % ai_names_c];
    struct Game* game = make_game(max_x, max_y, difficulty, sound, PADDLE_SENSITIVITY, first_name, second_name, 0, 0, DEFAULT_SCORE);
		...
		if(!g_queue_is_empty(shared_data)) {
			...
			switch (packet->code) {
				...
				case AUTH:
					if(packet->joystick == 1) {
						game->p1_aictrl = 0;
						game->p1_name = packet->username;
					} else {
						game->p2_aictrl = 0;
						game->p2_name = packet->username;
					}
